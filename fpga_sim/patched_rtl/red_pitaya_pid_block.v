...
// SIMULATION-ONLY PATCH (see fpga/fpga_sim/patched_rtl/README.md):
// with the default DERIVATIVE==0, this module-level kd_reg_s (referenced by
// the `pid_sum` continuous assignment below) is never driven by any
// always/assign -- both branches of the `generate if (DERIVATIVE==1) ...
// else ...` block below declare and drive their OWN same-named local
// kd_reg_s in a nested generate scope, which shadows but never connects to
// this one. An undriven `reg` simulates as X forever in Icarus (harmless in
// real synthesis, where it just optimizes to a constant 0), which poisons
// pid_sum/pid_out/dat_o with X for the entire run. This initial value is a
// no-op for real hardware (initial blocks are simulation-only) and exactly
// matches the intended behavior of a disabled differential path (contributes
// zero to pid_sum). Not present in the real repo file, which is untouched.
initial kd_reg_s = {30{1'b0}};
